...
internal unsafe class DebugVenture : DebugSectionBase
{
    ...
    public override void Draw()
    {
        ...
        ImGui.InputInt("委托 ID", ref VentureID);
        ImGui.InputText("委托名称", ref VentureName, 100);
        //if (ImGui.Button("SearchVentureByName")) DuoLog.Information(RetainerHandlers.SearchVentureByName(VentureName).ToString());
        if(ImGui.Button("清空委托列表")) DuoLog.Information(RetainerHandlers.ClearTaskSupplylist().ToString());
        if(ImGui.Button("按名称选择指定委托")) DuoLog.Information(RetainerHandlers.SelectSpecificVentureByName(VentureName).ToString());
        ...
        if(ImGui.CollapsingHeader("雇员派遣"))
        {
