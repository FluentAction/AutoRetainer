...
public unsafe class InventoryManagementCommon
{
    ...
    {
        if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.Copy, "复制到剪贴簿"))
        {
            Copy(EzConfig.DefaultSerializationFactory.Serialize(itemList, false));
        }
        ImGui.SameLine();
        if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.Paste, "与剪贴簿内容合并", ImGuiEx.Ctrl))
        {
            try
            {
        ...
        ImGuiEx.TreeNodeCollapsingHeader("批量新增/移除", () =>
        {
            ...
            if(ImGui.BeginCombo("选择类别", SelectedCategories.Count != 0 ? $"已选择 {SelectedCategories.Count}" : "未选择任何项目", ImGuiComboFlags.HeightLarge))
            {
                ...
                ImGui.SameLine();
                if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.Minus, "无"))
                {
                    SelectedCategories.Clear();
                    Modified = true;
                }
            ...
            {
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputText($"按名称过滤", ref ItemSearch, 100);
                ImGui.SetNextItemWidth(200f);
                if(ImGui.BeginCombo("选择稀有度", Rarities.Any() ? $"{Rarities.Print()}" : "任何稀有度", ImGuiComboFlags.HeightLarge))
                {
                ...
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("最低物品等级", ref ItemLevelMin);
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("最高物品等级", ref ItemLevelMax);
                ...
                if(ImGuiEx.CollapsingHeader($"已选择 {SelectedItems.Count} 个物品，其中 {SelectedItems.Count(x => itemList.Contains(x.RowId))} 个已在列表中###counter"))
                {
                    ...
                    foreach(var x in SelectedItems)
                    {
                        actions.Add(() =>
                        {
                            ...
                            void Tooltip()
                            {
                                ...
                                {
                                    if(ImGuiEx.HoveredAndClicked("点击立即将此物品加入清单"))
                                    {
                                        addAction(x.RowId);
                                    }
                                }
                                ...
                                {
                                    if(ImGuiEx.HoveredAndClicked("右键点击立即将此物品从清单移除", ImGuiMouseButton.Right))
                                    {
                ...
                if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.PlusSquare, "将这些物品加入清单", ImGuiEx.Ctrl))
                {
                    foreach(var x in SelectedItems)
                    {
                ...
                ImGuiEx.Tooltip("按住 CTRL 并点击");
                if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.MinusSquare, "将这些物品从清单移除", ImGuiEx.Ctrl))
                {
                    foreach(var x in SelectedItems)
                    {
                        removeAction(x.RowId);
    ...
    {
        if(ImGuiEx.Button("从 Discard Helper 导入丢弃项目", ImGuiEx.Ctrl))
        {
            try
            {
        ...
        ImGuiEx.HelpMarker("如果你有使用 Discard Helper 插件，可以点击此按钮导入项目。这些项目将与你现有的清单合并。请按住 CTRL 并点击。");
    }

    public void ImportBlacklistFromArDiscard()
    {
        var s = InventoryCleanupCommon.SelectedPlan;
        if(ImGuiEx.Button("从 Discard Helper 导入黑名单项目", ImGuiEx.Ctrl))
        {
            try
            {
                foreach(var x in EzConfig.LoadConfiguration<ARDiscardMiniConfig>(System.IO.Path.Combine(Svc.PluginInterface.ConfigDirectory.Parent.FullName, "ARDiscard.json")).DiscardingItems)
