...
{
    public override string Name => "背包清理/丢弃清单";
    ...
    private DiscardList()
    {
        Builder = InventoryCleanupCommon.CreateCleanupHeaderBuilder()
            .Section(Name)
            .TextWrapped("这些物品将始终被丢弃，不论其来源为何，只要其堆叠数量不超过下方可设置的数量。丢弃动作会非常频繁地发生，会在每次可能改变背包的操作前后进行。丢弃优先级最高，即使同一物品也存在于出售或分解清单中，也会被丢弃。已设置为保护的物品不会被丢弃。 ")
