...
internal class VentureBrowser : Window
{
    ...
    public override void Draw()
    {
        ...
        if(SelectedRetainer != null && SelectedCharacter != null)
        {
            ...
            {
                ImGuiEx.TextCentered($"{Lang.CharLevel}{SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)} | 物品等级: {adata.Ilvl} ({adata.Ilvl / (float)MaxPerception:P0})");
            }
            ...
            {
                ImGuiEx.TextWrapped($"缺少此雇员的数据。请与雇员铃互动并点选该雇员以填入数据。");
            }
            else
            {
                if(Data.Count == 0)
