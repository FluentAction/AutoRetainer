...
public sealed class VenturePlanner : Window
{
    ...
    public override void Draw()
    {
        ...
        if(ImGui.BeginCombo("##selectRet", $"{Censor.Character(SelectedCharacter.Name, SelectedCharacter.World)} - {Censor.Retainer(SelectedRetainer.Name)} - {SelectedRetainer.Level} {ExcelJobHelper.GetJobNameById(SelectedRetainer.Job)}" ?? "选择雇员...", ImGuiComboFlags.HeightLarge))
        {
        ...
        if(SelectedRetainer != null && SelectedCharacter != null)
        {
            ...
            if(adata.LinkedVenturePlan == "")
            {
                ...
                if(ImGui.Checkbox("启用规划器", ref adata.EnablePlanner))
                {
                ...
                {
                    ImGuiEx.SetNextItemFullWidth();
                    if(ImGui.BeginCombo("##load", "读取已储存的计划...", ImGuiComboFlags.HeightLarge))
                    {
                        int? toRem = null;
                ...
                {
                    //ImGui.Separator();
                    ImGuiEx.TextV("当计划完成时:");
                    ImGui.SameLine();
                    ImGuiEx.SetNextItemFullWidth();
                    ImGuiEx.EnumCombo("##cBeh", ref adata.VenturePlan.PlanCompleteBehavior, null, Lang.PlanCompleteBehaviorNames);
                    //ImGui.Separator();
                    var overwrite = C.SavedPlans.Any(x => x.Name == adata.VenturePlan.Name);
                    ImGuiEx.InputWithRightButtonsArea("储存计划", delegate
                    {
                    ...
                    }, delegate
                    {
                        if(ImGuiEx.IconButton(FontAwesomeIcon.Save))
                        {
                            ...
                            Notify.Success($"方案 {adata.VenturePlan.Name} 已保存！");
                        }
                        ImGuiEx.Tooltip(overwrite ? "覆盖现有的探险计划" : $"储存探险计划");
                    });
                }

                ImGuiEx.SetNextItemFullWidth();
                if(ImGui.BeginCombo("##addVenture", "新增探险任务...", ImGuiComboFlags.HeightLarge))
                {
                    ImGuiEx.SetNextItemFullWidth();
                    ...
                    ImGuiEx.TextV($"不可用的探险任务:");
                    ImGui.SameLine();
                    ImGuiEx.SetNextItemFullWidth();
                    ImGuiEx.EnumCombo("##unavail", ref C.UnavailableVentureDisplay, null, Lang.UnavailableVentureDisplayNames);
                    if(ImGui.BeginChild("##ventureCh", new(ImGui.GetContentRegionAvail().X, ImGuiHelpers.MainViewport.Size.Y / 3)))
                    {
                        ...
                        if(ImGui.Button($"{Lang.CharDice}    快速探险", ImGuiHelpers.GetButtonSize("A") with { X = ImGui.GetContentRegionAvail().X }))
                        {
